...
if __name__=='__main__':
    pwm = PWM(Pin(BL))
    pwm.freq(1000)
    pwm.duty_u16(32768) # 50% backlight.
    #pwm.duty_u16(65535) # 100% backlight. This will make your voltage regulator weirdly hot.
    ...
    LCD.write_text("ESP32 C6",10,8,2,LCD.WHITE)
    
    LCD.fill_rect(0,30,320,30,LCD.BLUE)
    LCD.write_text("WAVESHARE LCD 1.47\"",10,38,2,LCD.WHITE)
    
    LCD.fill_rect(0,60,320,30,LCD.GREEN)
    LCD.write_text("WHY DID I HAVE TO",10,68,2,LCD.WHITE)
    
    LCD.fill_rect(0,90,320,30,0X07FF)
    LCD.write_text("PORT THIS MYSELF",10,98,2,LCD.WHITE)
    
    LCD.fill_rect(0,120,320,30,0xF81F)
    LCD.write_text("THE LIBS USUALLY",10,128,2,LCD.WHITE)
